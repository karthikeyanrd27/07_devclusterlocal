...
var_topic_src_name = 'NBC_APPS.TBL_MS_AGENCY'
var_topic_tgt_name = 'NBC_APPS.TBL_MS_AGENCY_COMPACT1'
...
y = StructType([StructField("table",StringType(), nullable = True),
                ...
                StructField("AGENCY_ID",LongType(), nullable = False),
                StructField("AGENCY_NAME",StringType(), nullable = True),
                StructField("AGENCY_CREATION_DT",StringType(), nullable = True),
                StructField("AGENCY_MODIFICATION_DT",StringType(), nullable = True),
                StructField("PARENT_AGENCY_ID",LongType(), nullable = True),
...
def handler(message):
    ...
    for record in records:
        ...
        var_agency_id = var_loaded_r['AGENCY_ID']
...
# This part to Updated the Target Cassandra Table based on the updated records in reference table:
           ...
           if var_op_type =='U':
              ...
              ds = sqlContext \
                 ...
                 .load() \
                 .filter("AGENCY_ID=%d"%var_agency_id)
              ...
              inner_join = df.join(ds, df.AGENCY_ID == ds.AGENCY_ID).select(
                           ds.PLAN_ID,\
                           ds.op_type,\
                           ds.AGENCY_ID,\
                           df.AGENCY_NAME)
              ...
              if inner_join.rdd.isEmpty():
                 print("No data available for this Agency id  in cassandra table = %s"%var_agency_id)
              else:
