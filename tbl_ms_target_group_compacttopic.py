...
var_topic_src_name = 'NBC_APPS.TBL_MS_TARGET_GROUP'
var_topic_tgt_name = 'NBC_APPS.TBL_MS_TARGET_GROUP_COMPACT1'
...
y = StructType([StructField("table",StringType(), nullable = True),
                ...
                StructField("DEMO_ID",LongType(), nullable = False),
                StructField("DEMO_NAME",StringType(), nullable = True),
                StructField("AUDIENCE_ID",LongType(), nullable = True),
                StructField("FORMULA",StringType(), nullable = True),
                StructField("BLDNG_BLOCK_TYPE_CODE",LongType(), nullable = True),
                StructField("STEWARDSHIP_DEMO",LongType(), nullable = True),
...
def handler(message):
    ...
    for record in records:
        ...
        var_demo_id = var_loaded_r['DEMO_ID']
...
# This part to Updated the Target Cassandra Table based on the updated records in reference table:
           ...
           if var_op_type =='U':
              ...
              inner_join = df.join(ds, df.DEMO_ID == ds.DEMO_ID).select(
                           ds.PLAN_ID,\
                           ds.op_type,\
                           ds.DEMO_ID,\
                           df.DEMO_NAME)
              ...
              if inner_join.rdd.isEmpty():
                 print("No data available for this demo id  in cassandra table = %s"%var_demo_id)
              else:
