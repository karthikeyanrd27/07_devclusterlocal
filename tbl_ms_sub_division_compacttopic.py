...
var_topic_src_name = 'NBC_APPS.TBL_MS_SUB_DIVISION'
var_topic_tgt_name = 'NBC_APPS.TBL_MS_SUB_DIVISION_COMPACT1'
...
y = StructType([StructField("table",StringType(), nullable = True),
                ...
                StructField("CHANNEL_SUB_DIVISION_ID",LongType(), nullable = False),
                StructField("CHANNEL_SUB_DIVISION_NAME",StringType(), nullable = True),
                StructField("CHANNEL_DIVISION_ID",LongType(), nullable = True),
...
def handler(message):
    ...
    for record in records:
        ...
        var_ch_sub_division_id = var_loaded_r['CHANNEL_SUB_DIVISION_ID']
        var_op_type = var_loaded_r['op_type']
        print(var_ch_sub_division_id)
        print(var_op_type)
...
# This part to Updated the Target Cassandra Table based on the updated records in reference table:
           ...
           if var_op_type =='U':
              ...
              ds = sqlContext \
                 ...
                 .load() \
                 .filter("CHANNEL_SUB_DIVISION_ID =%d"%var_ch_sub_division_id)
              ...
              inner_join = df.join(ds, df.CHANNEL_SUB_DIVISION_ID == ds.CHANNEL_SUB_DIVISION_ID).select(
                           ds.PLAN_ID,\
                           ds.op_type,\
                           ds.CHANNEL_SUB_DIVISION_ID,\
                           df.CHANNEL_SUB_DIVISION_NAME)

              inner_join.show()
 
              if inner_join.rdd.isEmpty():
                 print("No data available for this chennal_sub_Division_id in cassandra table = %s"%var_ch_sub_division_id)
              else:
