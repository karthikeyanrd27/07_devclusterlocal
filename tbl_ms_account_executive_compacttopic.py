...
var_topic_src_name = 'NBC_APPS.TBL_MS_ACCOUNT_EXECUTIVE'
var_topic_tgt_name = 'NBC_APPS.TBL_MS_ACCOUNT_EXECUTIVE_COMPACT1'
...
y = StructType([StructField("table",StringType(), nullable = True),
                ...
                StructField("ACCOUNT_EXECUTIVE_ID",LongType(), nullable = False),
                StructField("ACCOUNT_EXECUTIVE",StringType(), nullable = True),
                StructField("CREATE_DATE",StringType(), nullable = True),
                StructField("SALES_TEAM_IDS",StringType(), nullable = True),
                ])
#----------------------------------------
...
def handler(message):
    ...
    for record in records:
        ...
        var_acct_executive_id = var_loaded_r['ACCOUNT_EXECUTIVE_ID']
        var_op_type = var_loaded_r['op_type']
        print(var_acct_executive_id)
        print(var_op_type)
...
# This part to Updated the Target Cassandra Table based on the updated records in reference table:
           ...
           if var_op_type =='U':
              ...
              ds = sqlContext \
                 ...
                 .load() \
                 .filter("ACCOUNT_EXECUTIVE_ID =%d"%var_acct_executive_id)
              ...
              inner_join = df.join(ds, df.ACCOUNT_EXECUTIVE_ID == ds.ACCOUNT_EXECUTIVE_ID).select(
                           ds.PLAN_ID,\
                           ds.op_type,\
                           ds.ACCOUNT_EXECUTIVE_ID,\
                           df.ACCOUNT_EXECUTIVE)

              inner_join.show()
 
              if inner_join.rdd.isEmpty():
                 print("No data available for this account executive id  in cassandra table = %s"%var_acct_executive_id)
              else:
