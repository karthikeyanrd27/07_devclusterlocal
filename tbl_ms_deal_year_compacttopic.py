...
var_topic_src_name = 'NBC_APPS.TBL_MS_DEAL_YEAR1'
var_topic_tgt_name = 'NBC_APPS.TBL_MS_DEAL_YEAR_COMPACT1'

producer = KafkaProducer(bootstrap_servers='localhost:9092')

y = StructType([StructField("table",StringType(), nullable = True),
                ...
                StructField("DEAL_YEAR_ID",LongType(), nullable = False),
                StructField("DEAL_YEAR_NAME",StringType(), nullable = True),
                StructField("CALENDAR_START_DATE",StringType(), nullable = True),
                StructField("CALENDAR_END_DATE",StringType(), nullable = True),
                StructField("BROADCAST_START_DATE",StringType(), nullable = True),
                StructField("BROADCAST_END_DATE",StringType(), nullable = True),
...
def handler(message):
    ...
    for record in records:
        ...
        var_deal_year_id = var_loaded_r['DEAL_YEAR_ID']
        var_op_type = var_loaded_r['op_type']
        print(var_deal_year_id)
        print(var_op_type)
...
# This part to Updated the Target Cassandra Table based on the updated records in reference table:
           ...
           if var_op_type =='U':
              ...
              ds = sqlContext \
                 ...
                 .load() \
                 .filter("DEAL_YEAR_ID =%d"%var_deal_year_id)
              ...
              inner_join = df.join(ds, df.DEAL_YEAR_ID == ds.DEAL_YEAR_ID).select(
                           ds.PLAN_ID,\
                           ds.op_type,\
                           ds.DEAL_YEAR_ID,\
                           df.DEAL_YEAR_NAME)
              ...
              if inner_join.rdd.isEmpty():
                 print("No data available for this deal Year id  in cassandra table = %s"%var_deal_year_id)
              else:
